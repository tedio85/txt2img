...
def dense(
	...
	with tf.variable_scope(name):
		W = tf.get_variable(name='W', shape=(x.get_shape()[-1], output_dim), initializer=W_init, dtype=dtype)
		b = tf.get_variable(name='b', shape=(output_dim), initializer=b_init, dtype=dtype)
...
def conv2d(
	...
	with tf.variable_scope(name):
		...
		b = tf.get_variable(name='b_conv2d', shape=filter_shape[-1], initializer=b_init, dtype=dtype)
...
def deconv2d(
	...
	with tf.variable_scope(name):
		...
		b = tf.get_variable(name='b_deconv2d', shape=filter_shape[-2], initializer=b_init, dtype=dtype)
		outputs = act(tf.nn.conv2d_transpose(x, W, output_shape=output_shape, strides=strides, padding=padding) + b)
	if with_all:
		return outputs, W, b
	else:
		return outputs
...
def batch_norm(
	...
	with tf.variable_scope(name):
        ...
        if is_train:
            ...
            outputs = act(tf.nn.batch_normalization(x, mean, var, beta, gamma, eps))
        else:
