...
if __name__ == '__main__':
    ...
    for _, item in df.iterrows():
        ...
        for cap in caps:
            ...
            im_w = Image.open(DIR_DATA + df['ImagePath'].iloc[np.random.randint(len(df))]).resize((500, 500))
            ...
            cnt += 1
            if cnt % 1000 == 0:
                print('finished: %d, time: %4.4fs' % (cnt, time.time() - st_time))


    X = pd.DataFrame(examples, columns=['image', 'caption', 'image_wrong', 'caption_wrong'])

    convert_to(X, 'train_data')
    print('Create %s, total Time: %4.4fs' %
          (DIR_DATA + 'train_data', time.time() - st_time))
